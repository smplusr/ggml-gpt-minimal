...
mkdir -p bin/models/gpt-2-$model

if [ -x "$(command -v wget)" ]; then
    wget --quiet --show-progress -O bin/models/gpt-2-$model/ggml-model.bin $src/$pfx-$model.bin
elif [ -x "$(command -v curl)" ]; then
    curl -L --output bin/models/gpt-2-$model/ggml-model.bin $src/$pfx-$model.bin
else
